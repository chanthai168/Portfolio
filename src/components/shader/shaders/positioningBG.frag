...
void main() {
    ...
    float glowNoise = max(fbm( vec2(10.0,10.0)  ), 0.2);
    glow = glowMask * glowNoise;
    glow = pow(glow,2.0);

    mouseOffset = (uv - mouseUv) * 2.0;
    ...
    vec2 distortion = vec2(fbm(uv * 1.5 + uTime * 0.05), fbm(uv * 1.5 + vec2(5.2, 1.3) + uTime * 0.05));

    vec2 distortedUV = uv + (distortion - 0.0 ) * 0.8;


    // add mouse distortion 
    distortedUV += mouseOffset * mouseInfluence * 0.2;
    ...
    color += glow * glowColor  * pulse;

    color*= 0.5; //decrease opacity

    color += colorGradient(uv) * 0.5 ;
